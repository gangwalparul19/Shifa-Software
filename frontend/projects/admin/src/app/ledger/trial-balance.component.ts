...
/**
 * Trial Balance (Reqs 14.1–14.4, 18.2). Read-only view for the ADMIN,
 * ACCOUNTANT, and CA roles (no mutations) that lists every ledger account with
 * an opening balance or a voucher line in the period, showing its closing
 * debit-or-credit magnitude, the debit/credit totals, and a prominent
 * balanced / difference banner.
 *
 * <p>The period is chosen either by financial year or by an explicit from/to
 * date range, mirroring the CA GST dashboard's period conventions.
 */
@Component({
  selector: 'admin-trial-balance',
  standalone: true,
...
export class TrialBalanceComponent implements OnInit {
  ...
  protected readonly data = signal<TrialBalance | null>(null);
  ...
  ngOnInit(): void {
    ...
    this.ledger.listFinancialYears().subscribe({
      ...
      error: () => {
        ...
        // default (current-FY) trial balance so the report is usable.
        this.load();
      },
    });
  }

  /** Load the trial balance for the currently selected period. */
  load(): void {
    this.loading.set(true);
    this.error.set(null);
    this.ledger.trialBalance(this.currentPeriod()).subscribe({
      next: (tb) => {
        this.data.set(tb);
        this.loading.set(false);
      },
      error: () => {
        this.error.set('Could not load the Trial Balance. Please check the period and try again.');
        this.loading.set(false);
      },
    });
  }
